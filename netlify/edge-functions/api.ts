...
app.post("/v1/chat/completions",async c=>{try{if(!authOk(c.req.raw))return err("Invalid API key",401,"authentication_error");const b=await c.req.json(),{response}=await call(b);const h=new Headers(response.headers);if(b.stream)streamHeaders(h);return new Response(response.body,{status:response.status,headers:h})}catch(e){return err(e instanceof Error?e.message:"Gateway error",500,"gateway_error")}});
